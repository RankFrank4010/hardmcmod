...
public class UrhuozhuiBiome extends SuperdifficultmodModElements.ModElement {
	public static Biome biome;

	public UrhuozhuiBiome(SuperdifficultmodModElements instance) {
		super(instance, 40);
		...
		public void registerBiomes(RegistryEvent.Register<Biome> event) {
			if (biome == null) {
				...
				BiomeGenerationSettings.Builder biomeGenerationSettings = new BiomeGenerationSettings.Builder().withSurfaceBuilder(
						SurfaceBuilder.DEFAULT.func_242929_a(new SurfaceBuilderConfig(Blocks.YELLOW_CONCRETE_POWDER.getDefaultState(),
								Blocks.OAK_LOG.getDefaultState(), Blocks.WATER.getDefaultState())));
				...
				mobSpawnInfo.withSpawner(EntityClassification.CREATURE, new MobSpawnInfo.Spawners(MyfriendEntity.entity, 20, 5, 30));
				biome = new Biome.Builder().precipitation(Biome.RainType.RAIN).category(Biome.Category.NONE).depth(0.1f).scale(0.2f).temperature(0.5f)
						.downfall(0.5f).setEffects(effects).withMobSpawnSettings(mobSpawnInfo.copy())
						.withGenerationSettings(biomeGenerationSettings.build()).build();
				event.getRegistry().register(biome.setRegistryName("superdifficultmod:urhuozhui"));
			}
		}
	}
